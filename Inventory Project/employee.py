...
class employeeClass:
    def __init__(self,root):
        ...
        txt_password=Entry(self.root, textvariable=self.var_password, font=("goudy old style",15),bg="white").place(x=500, y=230, width=180)
        ...
        txt_salary=Entry(self.root, textvariable=self.var_salary, font=("goudy old style",15),bg="white").place(x=600, y=270, width=180)

        #====buttons======
        btn_add=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="black",cursor="hand").place(x=500,y=305,width=110,height=28,)
        btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="black",cursor="hand").place(x=620,y=305,width=110,height=28,)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="#f44336",fg="black",cursor="hand").place(x=740,y=305,width=110,height=28,)
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="black",cursor="hand").place(x=860,y=305,width=110,height=28,)
...
#==============================================================
    ...
    def add(self):
        ...
        try:
           if self.var_emp_id.get()=="" :
               messagebox.showerror("Error","Employee ID must be ENTERED!",parent=root)
           else:
               cur.execute("Select * from employee where eid=?",(self.var_emp_id.get(),))
               row=cur.fetchone()
               if row !=None:
                   messagebox.showerror("Error","This employee ID is already taken, try a different one",parent=self.root)
               else:
        ...
        try:
            if self.var_emp_id.get()=="" :
               ...
               #   if op==True:
                    ...
                    # messagebox.showinfo("Delete","Employee Deleteted Successfully",parent=self.root)
                    # self.show()
               if row ==None:
                   messagebox.showerror("Error","Invalid Employee ID",parent=self.root)
               else:
    ...
    def clear(self):
        ...
        self.var_searchtext.set("Select")
        self.clear()
